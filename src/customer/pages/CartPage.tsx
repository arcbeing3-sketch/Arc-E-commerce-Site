  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 pb-6">
        <div>
          <span className="text-xs font-bold tracking-widest uppercase text-zinc-400">ARC Bag Review</span>
          <h1 className="text-3xl font-extrabold text-zinc-900 mt-1">Shopping Bag ({cartCount} items)</h1>
        </div>
        <button
          onClick={onNavigateToCatalog}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          {/* Free Shipping Alert */}
          <div className="bg-zinc-900 text-white p-4 rounded-2xl text-xs space-y-2">
          ...
          {/* Items Table */}
          <div className="bg-white border border-zinc-200 rounded-2xl divide-y divide-zinc-200 overflow-hidden shadow-2xs">
            {cart.map((item) => {
                ...
                <div key={item.product.id} className="p-4 sm:p-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                  <div className="flex items-center gap-4">
                    <img
                      ...
                      className="w-20 h-20 rounded-xl object-cover bg-zinc-100 shrink-0 cursor-pointer"
                    />
                    <div className="space-y-1">
                      ...
                      <h3
                        onClick={() => onNavigateToProduct(item.product.id)}
                        className="text-sm font-bold text-zinc-900 hover:text-zinc-600 cursor-pointer line-clamp-1"
                      >
                        {item.product.name}
                      </h3>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 pt-3 sm:pt-0 border-t sm:border-t-0 border-zinc-100">
                    ...
                    {/* Total for item */}
                    <span className="text-sm font-extrabold text-zinc-900 min-w-24 text-right">
        ...
        {/* Right: Order Summary */}
        <div className="lg:col-span-4 bg-zinc-50 border border-zinc-200/80 rounded-3xl p-6 space-y-6 shadow-2xs">
          <h2 className="text-lg font-bold text-zinc-900 border-b border-zinc-200 pb-4">
            Order Summary
          </h2>
          ...
          <button
            ...
            className="w-full py-3.5 px-4 bg-zinc-900 hover:bg-black text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md flex items-center justify-center gap-2 hover:shadow-lg"
          >
